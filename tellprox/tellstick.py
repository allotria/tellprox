...
class TellstickAPI(object):
    ...
    def __init__(self, api, config, version):
        ...
        api.add_route('clients', {
            'list': {
                ...
                'inputs': {'name': 'extras', 'type': 'string',
        ...
        api.add_route('sensors', {
            'list': {
                ...
                'inputs': {'name': 'includeignored', 'type': 'int',
    ...
    def run_command(self, device, func, value, method, lastCommand):
        if (method == TELLSTICK_BELL or func == 'bell'):
            device.bell()
        elif (method == TELLSTICK_DIM or func == 'dim'):
            device.dim(value)
        elif (method == TELLSTICK_DOWN or func == 'down'):
            device.down()
        elif (method == TELLSTICK_LEARN or func == 'learn'):
            device.learn()
        elif (method == TELLSTICK_STOP or func == 'stop'):
            device.stop()
            ...
            return "Device " + str(id) + " does not support method " + str(method)
        return True
